...
public class ControllerGenerator {
    ...
    public static void generatorController(String path, String className, String controllerPackage){
        try{
            ...
            res.append("\t@Autowired(required = false)\n");
            res.append("\tprivate "+ className +"Service "+ lowerClassName +"Service;\n\n");

            res.append("\t/** 多条件分页查询方法 */\n");
            res.append("\t@GetMapping(\"/findByPage\")\n");
            res.append("\tpublic List<"+className+"> save("+ className +" " + lowerClassName + ",\n");
            res.append("\t\t\t@RequestParam(value=\"page\", defaultValue=\"1\")Integer page,\n");
            res.append("\t\t\t@RequestParam(value=\"rows\", defaultValue=\"10\")Integer rows) {\n");
            res.append("\t\ttry {\n");
            res.append("\t\t\tList<"+ className +"> "+ lowerClassName +"List = "+ lowerClassName +"Service.findByPage("+ lowerClassName +", page, rows);\n");
            res.append("\t\t\treturn "+ lowerClassName +"List;\n");
